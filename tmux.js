...
wss.on('connection', function connection (ws) {
  ...
  if (match = ws.upgradeReq.url.match(/.*?\/tmux\?token=(.*)$/)) {
    qtoken = match[1];
    console.log('qtoken',qtoken,token);
  } 
  ...
  if (process.env.SESSION != undefined) {
    args = ['-t','localhost','-o','StrictHostKeyChecking=no','-o','UserKnownHostsFile=/dev/null','-o','LogLevel=ERROR','tmux attach-session -t'+process.env.SESSION]
  } else {
    args = ['-t','localhost','-o','StrictHostKeyChecking=no','-o','UserKnownHostsFile=/dev/null','-o','LogLevel=ERROR','bash']
  }
